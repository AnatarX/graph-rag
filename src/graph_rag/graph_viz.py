...
# barnesHut — силовая раскладка vis.js: узлы отталкиваются друг от друга, рёбра стягивают
# как пружины. Даёт органичный "разлёт" с видимыми кластерами хабов вместо равномерного
# кольца по периметру, которое давал предыдущий статичный `spring_layout` (Fruchterman-
# Reingold без анкеров тяготеет ставить слабосвязанные узлы по окружности). Физику
# выключаем сразу после стабилизации (см. `_freeze_after_stabilize`) — иначе на графе
# в сотни узлов Streamlit-ререндер каждый раз заново "трясёт" узлы несколько секунд, а
# перетаскивание/зум после заморозки продолжают работать как обычно.
_PHYSICS_OPTIONS = {
    "edges": {"smooth": False},
    "physics": {
        "enabled": True,
...
def build_pyvis_html(G: nx.MultiDiGraph, keys: set[str] | None = None, height: str = "600px") -> str:
    ...
    более естественный разлёт по кластерам, чем статичный spring_layout. Физика
    автоматически замораживается, как только раскладка стабилизируется (см.
