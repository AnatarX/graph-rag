"""Кластеризация датасета по эмбеддингам + человекочитаемые названия кластеров через LLM.
...
KMeans выбран вместо HDBSCAN, потому что: (1) датасет маленький и плотный после
...
датасете, где часть документов не должна принудительно попадать ни в один кластер —
